...
upsertBlock(
  ...
  let scrollX=0,vel=0,startX=0,startScrollX=0,pointerId=null;
  let lastT=0,lastX=0,momentumRaf=null,tiltRaf=null;
  ...
  const applyScroll=x=>{const h=handEl();if(!h)return;scrollX=x;h.style.setProperty('--hand-scroll-x',x.toFixed(1)+'px');updateOverflowHint();};
  ...
  const runMomentum=()=>{
    let v=vel*16;
    if(Math.abs(v)<.4){easeTiltTo(0);return;}
    const step=()=>{
      const next=clampScroll(scrollX+v);
      if(Math.abs(next-scrollX)<.2||Math.abs(v)<.25){momentumRaf=null;easeTiltTo(0);applyScroll(clampScroll(scrollX));return;}
      applyScroll(next);
      setTilt(Math.max(-3.5,Math.min(3.5,-v*.22)));
      v*=.92;
  ...
  document.addEventListener('pointerdown',ev=>{
    ...
    lastT=performance.now();lastX=ev.clientX;vel=0;
  ...
  document.addEventListener('pointermove',ev=>{
    ...
    const next=clampScroll(startScrollX+dx);
    applyScroll(next);
    const now=performance.now(),dt=now-lastT;
    if(dt>0){const inst=(ev.clientX-lastX)/dt;vel=vel*.55+inst*.45;}
    lastT=now;lastX=ev.clientX;
    setTilt(Math.max(-4.5,Math.min(4.5,-vel*8)));
