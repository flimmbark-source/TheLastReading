...
// Pointer-driven swipe handler: rotation around the hand's center, with momentum.
upsertBlock(
  ...
  const clampRot=d=>Math.max(-ROT_MAX,Math.min(ROT_MAX,d));
  const softClamp=d=>{const ad=Math.abs(d);if(ad<=ROT_MAX)return d;return Math.sign(d)*(ROT_MAX+(ad-ROT_MAX)*RUBBER);};
  const applyRot=d=>{const h=handEl();if(!h)return;rot=d;h.style.setProperty('--hand-rotate',d.toFixed(2)+'deg');updateOverflowHint();};
  const updateOverflowHint=()=>{const z=zoneEl();if(!z)return;const h=handEl();const over=h&&h.parentElement?(h.scrollWidth>h.parentElement.clientWidth+4):false;z.classList.toggle('has-overflow',over);};
  ...
  const releaseVel=()=>{
    ...
    // doesn't bleed an older swipe's velocity into the momentum.
  ...
  const springBack=()=>{
    const target=clampRot(rot);
    if(Math.abs(target-rot)<.05){applyRot(target);return;}
    const t0=performance.now(),from=rot,dur=340;
    const step=t=>{
      const p=Math.min(1,(t-t0)/dur),e=1-Math.pow(1-p,3);
      applyRot(from+(target-from)*e);
      if(p<1)momentumRaf=requestAnimationFrame(step);else momentumRaf=null;
    };
    momentumRaf=requestAnimationFrame(step);
  };
  const runMomentum=v0=>{
    if(Math.abs(rot)>ROT_MAX+.05){springBack();return;}
    let v=v0;
    ...
    const step=now=>{
      ...
      let next=rot+v*dt;
      if(next<-ROT_MAX){next=-ROT_MAX;v=0;}
      else if(next>ROT_MAX){next=ROT_MAX;v=0;}
      applyRot(next);
      if(Math.abs(v)<MIN_VEL){applyRot(clampRot(rot));momentumRaf=null;return;}
      momentumRaf=requestAnimationFrame(step);
    };
    momentumRaf=requestAnimationFrame(step);
  };
  document.addEventListener('pointerdown',ev=>{
    ...
    startX=ev.clientX;startRot=rot;
    samples.length=0;pushSample(performance.now(),rot);
  ...
  document.addEventListener('pointermove',ev=>{
    ...
    const target=softClamp(startRot+dx*DEG_PER_PX);
    applyRot(target);
  ...
  const recheck=()=>{applyRot(clampRot(rot));};
  let ro=null;
