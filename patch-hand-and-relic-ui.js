...
// Pointer-driven swipe handler: slide the cards along the track (horizontal translation).
upsertBlock(
  ...
  let slide=0,startSlide=0,startX=0,pointerId=null;  // slide/startSlide in px
  let samples=[];                                    // {t,x} ring of recent pointer samples
  const SAMPLE_WINDOW=90;          // ms window for swipe velocity sampling
  const FRICTION=0.0028;           // exponential decay per ms during momentum
  const MIN_VEL=0.012;             // px/ms threshold to stop momentum
  const RUBBER=0.42;               // resistance past edge while dragging
  ...
  const recheck=()=>{applySlide(clampSlide(slide));};
  let ro=null;
