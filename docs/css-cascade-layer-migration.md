...
| `mpSinglePlayerIsolation.css` | layer-move, **since deleted entirely** | was before `legacy` | The third crack, and a different shape from the first two — not property-disjoint, just dead. Every rule in the file was gated by `body.mp-game-active.single-player-v2` (all of them, no exception), and those two classes are enforced mutually exclusive at runtime by `src/app/mpModeClassGuard.mjs`'s `MutationObserver`: the instant `mp-game-active` is added, `single-player-v2` is stripped on the same microtask, before the next paint. No other file in the app referenced this combined gate. So the file's entire ruleset was unreachable in any rendered frame, and its layer position genuinely couldn't affect real behavior regardless of where it was declared. Verified at the time by reading computed style in the actual transient window itself — set all three classes in one synchronous assignment and read immediately, before the guard's microtask could run (confirmed via `document.body.className` in the probe output: all three classes were still present at read time) — a full computed-style A/B diff on synthetic `#spread`/`#mpOppSpread`/`.handDock`/`#handSwipeZone` elements came back byte-for-byte identical against the pre-extraction baseline even in that window. **Deleted outright once every other file in the app had its own reasoned layer position too** (see the `spv2Core` row above, the last piece to land): keeping a whole 59-`!important`, 134-line file around purely to document that its position didn't matter stopped being worth it. Its stylesheet link and layer name were removed from `game.html`'s master statement, and its 59 declarations came out of the app-wide `!important` budget (711 → 652, a real reduction, not a tracking-gap correction) — see `scripts/validate-app-important-budget.mjs`. Re-verified empirically one last time before deletion: removing the file produced zero computed-style diff anywhere reachable, confirming it really was fully inert. |
...
- The multiplayer cluster is now fully extracted (`mpMultMobile.css`,
  `mpSpreadCards.css`, `components/mpGameChrome.css`, and `mpCore` — see
  Done table above; `mpSinglePlayerIsolation.css`, the fifth piece, turned
  out to be confirmed-dead code and was deleted outright rather than
  extracted — see its own Done-table row for the full writeup). One rule
  remains behind in `legacy` on purpose: `mpMobile.css`'s
...
16. **A same-tick computed-style read is not the same thing as "reachable
    in a rendered frame," and an A/B probe that reads synchronously right
    after setting mutually-exclusive mode classes will report a false
    regression for code that's actually dead.** While verifying the
    deletion of `mpSinglePlayerIsolation.css` (see its Done-table row),
    setting `body.mp-game-active` and `body.single-player-v2` together in
    one synchronous assignment and reading `getComputedStyle` immediately
    (mirroring the technique used to originally confirm the file was dead)
    showed a real difference between the file present and deleted —
    seemingly contradicting the "unreachable in any rendered frame"
    verdict. It didn't: `MutationObserver` callbacks (including
    `mpModeClassGuard.mjs`'s correction) are microtasks, and microtasks
    always fully drain before the browser paints a frame or runs the next
    macrotask (event handler, `setTimeout`, `requestAnimationFrame`) — so
    a synchronous script-level read performed in the same tick as the
    class assignment can observe the rule matching, but no actual painted
    frame or later-running application code ever does, unless that code
    also reads layout synchronously in that same tick (traced through
    `src/app/mpGame.mjs`'s real `tlrMpOnMatchStart` → `mount()`/`render()`
    call chain to confirm none of it reads `getComputedStyle`/
    `getBoundingClientRect` there — only `innerHTML`/`classList` writes).
    Re-running the probe with a few `await Promise.resolve()` turns
    inserted before reading (letting the microtask queue drain the same
    way real code implicitly does before its next opportunity to read
    layout) came back byte-for-byte identical, confirming the deletion was
    safe after all. The general lesson: when verifying code gated by
    classes an `MutationObserver`-based guard corrects, a probe that reads
    state in the exact same synchronous tick as the triggering mutation is
    testing a window no real code path can observe — yield past the
    microtask queue first, or trace the real call site to confirm it
    doesn't read layout synchronously either.
