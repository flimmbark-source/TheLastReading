...
assert.ok(fxSource.includes('CARD_DURATION_MS=1620'),'activation has an additional second to read');
assert.ok(fxSource.includes('BURST_DELAY_MS=1200'),'sparkle bloom waits for the final part of the flight');
...
assert.equal(animationOptions[0].duration,1620,'card presentation includes the added second');
...
assert.equal(animationOptions[1].delay,1200,'sparkle bloom begins during the final part of the flight');
assert.equal(animationOptions[1].delay+animationOptions[1].duration,animationOptions[0].duration,'sparkle bloom and card flight finish together');
