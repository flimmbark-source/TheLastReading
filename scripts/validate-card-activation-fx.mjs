...
assert.ok(fxSource.includes('CARD_DURATION_MS=1200'),'activation lasts 1.2 seconds');
assert.ok(fxSource.includes('BURST_DELAY_MS=780'),'sparkle bloom waits for the final part of the flight');
...
assert.equal(animationOptions[0].duration,1200,'card presentation lasts 1.2 seconds');
...
assert.equal(animationOptions[1].delay,780,'sparkle bloom begins during the final part of the flight');
