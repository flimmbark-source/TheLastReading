...
assert.ok(fxSource.includes('CARD_DURATION_MS=700'),'activation lasts 0.7 seconds');
assert.ok(fxSource.includes('BURST_DELAY_MS=380'),'sparkle bloom waits for the final part of the flight');
...
assert.equal(animationOptions[0].duration,700,'card presentation lasts 0.7 seconds');
...
assert.equal(animationOptions[1].delay,380,'sparkle bloom begins during the final part of the flight');
assert.equal(animationOptions[1].delay-100+animationOptions[1].duration,animationOptions[0].duration,'sparkle bloom and card flight finish together');
