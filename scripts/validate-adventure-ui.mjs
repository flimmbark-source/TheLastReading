...
assert.ok(el.querySelector('.adv-handdock .adv-hand'), 'has a hand dock');
assert.equal(el.querySelectorAll('.adv-handdock .adv-hand .card').length, 5, 'hand is dealt (5 cards)');
assert.ok(el.querySelector('[data-act=cast]').disabled, 'cannot cast an empty spread');
// The four bottom utility medallions (Menu / Archive / Scoring / Abilities).
...
// No stray discard/remove action buttons on the event screen.
assert.ok(!el.querySelector('[data-act=discard]'), 'no extra discard button');
assert.ok(!el.querySelector('[data-act=purge]'), 'no extra purge button');
...
assert.ok(overlay().querySelector('.adv-handdock .adv-hand'), 'closing a panel returns to the event');

function playOneEvent() {
  // Tap each hand card to lay it into the next open slot.
  for (let i = 0; i < 5; i += 1) {
    const card = overlay().querySelector('.adv-handdock .adv-hand .card');
...
while (guard < 12) {
  ...
  if (overlay().querySelector('.adv-handdock .adv-hand')) { playOneEvent(); continue; }
  break;
}
