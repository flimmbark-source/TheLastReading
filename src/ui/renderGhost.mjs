...
function spark(x,y,color,n=6,spread=36,size=5){if(reducedMotion())return;for(let k=0;k<n;k++){const p=document.createElement('span');const a=Math.random()*Math.PI*2,d=spread*(.5+Math.random()*.7),s=size*(.6+Math.random()*.8);p.style.cssText=`position:fixed;left:${x}px;top:${y}px;width:${s}px;height:${s}px;border-radius:50%;background:${color};box-shadow:0 0 6px ${color};pointer-events:none;z-index:99998`;document.body.appendChild(p);p.animate([{transform:'translate(-50%,-50%) scale(1)',opacity:1},{transform:`translate(calc(-50% + ${(Math.cos(a)*d).toFixed(1)}px),calc(-50% + ${(Math.sin(a)*d-14).toFixed(1)}px)) scale(.2)`,opacity:0}],{duration:450+Math.random()*350,easing:'cubic-bezier(.1,.7,.3,1)'}).onfinish=()=>p.remove()}}

function shockwave(x,y,color){if(reducedMotion())return;const w=document.createElement('div');w.style.cssText=`position:fixed;left:${x}px;top:${y}px;width:14px;height:14px;border:2px solid ${color};border-radius:50%;pointer-events:none;z-index:99997;transform:translate(-50%,-50%)`;document.body.appendChild(w);w.animate([{transform:'translate(-50%,-50%) scale(.4)',opacity:.9},{transform:'translate(-50%,-50%) scale(5)',opacity:0}],{duration:480,easing:'cubic-bezier(.1,.6,.3,1)'}).onfinish=()=>w.remove()}

let _shakeAnim=null;
export function screenShake(power=3){if(reducedMotion()||!document.body.animate)return;if(_shakeAnim)_shakeAnim.cancel();_shakeAnim=document.body.animate([{transform:'translate(0,0)'},{transform:`translate(${power}px,${-power*.6}px)`},{transform:`translate(${-power*.8}px,${power*.5}px)`},{transform:`translate(${power*.5}px,${power*.4}px)`},{transform:'translate(0,0)'}],{duration:220,easing:'ease-out'});_shakeAnim.onfinish=()=>{_shakeAnim=null}}

export function scorePillFlash(){const pill=document.querySelector('.score-stack .score-pill');if(!pill||!pill.animate||reducedMotion())return;const r=pill.getBoundingClientRect();spark(r.left+r.width/2,r.top+r.height/2,'#ffd978',8,44,4);pill.animate([{boxShadow:'0 0 0 0 rgba(255,217,120,0)',filter:'brightness(1)'},{boxShadow:'0 0 20px 5px rgba(255,217,120,.75)',filter:'brightness(1.5)'},{boxShadow:'0 0 0 0 rgba(255,217,120,0)',filter:'brightness(1)'}],{duration:550,easing:'ease-out'})}

export function ghost(i,t,big=false,relicKey=null){const s=_slots()[i];if(!s)return;holdEffects(1700);let g=document.createElement('div');g.className='ghost '+(big?'big':'');g.style.setProperty('--dx',(Math.random()*20-10).toFixed(1)+'px');g.style.setProperty('--rot',(Math.random()*8-4).toFixed(1)+'deg');if(relicKey){const ic=document.createElement('span');ic.style.cssText=`display:inline-block;width:14px;height:14px;vertical-align:middle;margin-right:3px;${relicIconStyle(relicKey,14)}`;g.appendChild(ic);g.appendChild(document.createTextNode(t));}else{g.textContent=t;}const r=s.getBoundingClientRect();g.style.position='fixed';g.style.left=(r.left+r.width/2)+'px';g.style.top=(r.top-10)+'px';g.style.zIndex='99999';document.body.appendChild(g);const gx=r.left+r.width/2,gy=r.top-10;if(big){spark(gx,gy,'#ff9b52',12,55,5);shockwave(gx,gy,'#ff9b52');screenShake(3);}else{spark(gx,gy,'#70d37d',5,30,4);}const card=s.querySelector('.card');if(card&&card.animate&&!reducedMotion())card.animate([{filter:'brightness(1)'},{filter:'brightness(1.55)'},{filter:'brightness(1)'}],{duration:240,easing:'ease-out'});setTimeout(()=>g.remove(),1700)}

export function centerGhost(name,rare=false){const g=document.createElement('div');g.className='meld-announce'+(rare?' rare':'');g.textContent=name;document.body.appendChild(g);if(rare){const cx=innerWidth/2,cy=innerHeight*.42;spark(cx,cy,'#ffd978',14,80,5);shockwave(cx,cy,'#ffd978');screenShake(4);}setTimeout(()=>g.remove(),1900)}
...
export function fireMultGhost(label){const pill=document.querySelector('.score-stack .score-pill');if(!pill)return;const r=pill.getBoundingClientRect();const g=document.createElement('span');g.className='score-ghost mult';g.textContent=label;g.style.left=(r.left+8+Math.random()*(r.width-16))+'px';g.style.top=(r.top+r.height*0.25)+'px';document.body.appendChild(g);spark(parseFloat(g.style.left),r.top+r.height*0.25,'#ff5a5a',6,34,4);if(pill.animate&&!reducedMotion())pill.animate([{filter:'brightness(1)'},{filter:'brightness(1.7)'},{filter:'brightness(1)'}],{duration:280,easing:'ease-out'});setTimeout(()=>g.remove(),950)}
