...
function ensureStoreFrontStyles(target = window) {
  ...
  style.textContent = `
    ...
    .store-offer-row{position:absolute;left:17.2%;right:8.5%;top:28%;bottom:12%;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9.5%}.store-section.abilities .store-offer-row{top:24%;bottom:15%}.store-scoring-row{position:absolute;left:7.2%;right:3.8%;top:25%;bottom:10%;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:4.2%}.store-relic-row{position:absolute;left:15.3%;right:5.2%;top:24%;bottom:11%;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5.5%}
...
function renderStoreScoringSection(upgradeKeys, target = window) {
  ...
  return `<section class="store-section scoring">
    ...
    <div class="store-section-copy">Upgrade scoring patterns directly.</div>
