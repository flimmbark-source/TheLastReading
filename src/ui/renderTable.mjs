...
function renderConstellationPill(){installConstellationOutsideHandler();const el=ensureConstellationPill();const constellation=getConstellation(state.constellationId);if(!constellation){el.classList.add('hidden');el.innerHTML='';closeConstellationCallout();return}const setText='Set '+((state.setIndex||0)+1)+'/'+(state.setsPerRound||2);const scoreText='Round '+(state.roundScore||0)+'/'+activeThreshold();el.classList.remove('hidden');el.setAttribute('aria-label',`${constellation.name}. ${constellation.shortRule||constellation.rule}`);el.innerHTML=constellationIconHTML(constellation);positionConstellationIcon(el);el.onclick=event=>{event.stopPropagation();if(constellationCalloutOpen)closeConstellationCallout();else showConstellationCallout(el,constellation,setText,scoreText)}}
