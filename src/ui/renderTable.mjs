...
function hasActiveConstellation(){return Number(state.th||0)>0&&!!state.constellationId}
...
function constellationIconHTML(c,extraClass=''){if(c?.sprite)return`<span class="constellation-sprite ${extraClass}" aria-hidden="true" style="${constellationSpriteStyle(c)}"></span>`;return`<span class="constellation-icon ${extraClass}" aria-hidden="true">${escapeHTML(c?.icon||'✦')}</span>`}
...
function renderConstellationPill(){installConstellationOutsideHandler();const el=ensureConstellationPill();if(!hasActiveConstellation())return hideConstellationPill(el);const constellation=getConstellation(state.constellationId);if(!constellation||!constellation.sprite)return hideConstellationPill(el);const setText='Set '+((state.setIndex||0)+1)+'/'+(state.setsPerRound||2);const scoreText='Round '+(state.roundScore||0)+'/'+activeThreshold();el.classList.remove('hidden');el.setAttribute('aria-label',`${constellation.name}. ${constellation.shortRule||constellation.rule}`);el.innerHTML=constellationIconHTML(constellation);positionConstellationIcon(el);el.onclick=event=>{event.stopPropagation();if(constellationCalloutOpen)closeConstellationCallout();else showConstellationCallout(el,constellation,setText,scoreText)}}
