...
export function installPresentationDirector(target = window) {
  ...
  const syncFromDom = () => {
    ...
    if (placing && !domState.placing) cue('card-place', { duration: 280, intensity: .25 });
    if (patterning && !domState.patterning) cue('pattern', { duration: 700, intensity: .65 });
    ...
    if (adventureOutcome && !domState.adventureOutcome) cue('adventure-outcome', { duration: 620, intensity: .65 });
    if (adventureReward && !domState.adventureReward) cue('adventure-reward', { duration: 620, intensity: .7 });
    if (adventureRecovery && !domState.adventureRecovery) cue('adventure-recovery', { duration: 620, intensity: .45 });
