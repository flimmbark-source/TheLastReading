...
const TUT_ADV_APPROACH_TIERS_KEY = 'tlr_tut_adv_approach_tiers';
...
export const TUT_STEP = Object.freeze({
  ...
  ADVENTURE_APPROACH_TIERS: 31,
  ADVENTURE_REWARD: 32,
  ADVENTURE_ITEMS: 33,
  ADVENTURE_COMPLETE: 34,
});
...
const TUT_STEPS = [
  ...
  { center: true, key: TUT_ADV_APPROACH_TIERS_KEY, text: 'Each Event has three accepted approaches at easy, medium, and hard difficulty. Harder approaches need a higher red number to pass.' },
...
export function replayTutorial() {
  if (window.__tlrAdventureActive) {
    [TUT_ADVENTURE_KEY, TUT_ADV_APPROACH_KEY, TUT_ADV_APPROACH_CHAIN_KEY, TUT_ADV_APPROACH_GREAT_KEY, TUT_ADV_APPROACH_TIERS_KEY, TUT_ADV_REWARD_KEY, TUT_ADV_ITEMS_KEY, TUT_ADV_COMPLETE_KEY].forEach(k => localStorage.removeItem(k));
    queuedTipSteps = [];
    clearTimeout(queuedTipTimer);
    queuedTipTimer = null;
    tutHide();
...
export function tutSignal(eventName) {
  ...
  if (eventName === 'advApproachWebOpened') { queueTip(TUT_STEP.ADVENTURE_APPROACH_WEB, 300); queueTip(TUT_STEP.ADVENTURE_APPROACH_CHAIN, 300); queueTip(TUT_STEP.ADVENTURE_APPROACH_GREAT, 300); queueTip(TUT_STEP.ADVENTURE_APPROACH_TIERS, 300); return; }
