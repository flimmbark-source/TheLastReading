...
function bundleList(persist) {return (persist.pendingRewardBundles || []).filter(bundle => bundle.state !== 'claimed');}
function syncStorePersistToLegacy(target = window) {const storePersist = target.tlrStore?.getState?.()?.persist;if (storePersist) copyStorePersistToLegacy(storePersist, persistOf(target));}
...
function renderBundleCard(bundle) {
  ...
  return `<div class="store-card store-card--bundle ${accent}"><div class="store-card-tag">${escapeHtml(display.categoryLabel || 'Bundle')}</div><div class="store-card-art"><span class="isp isp-108 ${escapeHtml(icon)}"></span></div><div class="store-card-main"><div class="store-card-name">${escapeHtml(name)}</div><div class="store-card-desc">${escapeHtml(desc)}</div></div><button class="store-card-buy" onclick="openRewardBundleWithAnimation('${escapeHtml(bundle.id)}')">${stateLabel}</button></div>`;
}

function renderQuietMarketCard(){return `<div class="store-card store-card--empty disabled"><div class="store-card-tag">Market</div><div class="store-card-art"><span class="isp isp-108 isp-scoring"></span></div><div class="store-card-main"><div class="store-card-name">The Market Is Quiet</div><div class="store-card-desc">No reward bundles are waiting.</div></div></div>`;}

function bundleMarketInner(target = window) {
  const persist = persistOf(target);
  const bundles = bundleList(persist);
  const hasBundles = bundles.length > 0;
  return `<div class="store-meta"><div aria-hidden="true"></div><div class="store-reserve-display"><div class="store-reserve-label">Bundles</div><div class="store-reserve-amount">${bundles.length}</div></div></div><div class="store-offer-row">${hasBundles ? bundles.map(renderBundleCard).join('') : renderQuietMarketCard()}</div><div class="store-footer"><button class="store-proceed" onclick="storeExitToNextReading()">Next Reading →</button></div>`;
}

function showBundleMarket(target = window) {
  ensureBundleStyles(target);syncStorePersistToLegacy(target);
  const doc = target.document;
  ...
  const html = `<div class="summary store-front-shell"><div class="store-dim"></div><div class="store-candle${alreadyOpen ? ' lit' : ''}" id="storeCandle"><img class="candle-off" src="ui/candle_flame_off.small.webp" alt=""><img class="candle-on" src="ui/candle_flame_on.small.webp" alt=""></div><div class="store-front${alreadyOpen || reduce ? ' store-visible' : ''}" id="storeFront">${bundleMarketInner(target)}</div></div>`;
...
function openBundleMarket(target = window) {if (typeof target.enterMarket === 'function') target.enterMarket();clearSpreadForBundleMarket(target);return showBundleMarket(target);}
function poolSetForBundle(bundle) {const pools = MARKET_BUNDLE_REWARD_POOLS[bundle?.bundleId];if (!pools) return null;return bundle.tier > 1 ? pools.tier2 || pools.tier1 || null : pools.tier1 || null;}
function rewardRoleForKey(bundle, rewardKey) {const pools = poolSetForBundle(bundle);if (!pools) return 'reward';for (const role of ['core', 'tool', 'bridge']) if ((pools[role] || []).includes(rewardKey)) return role;return 'reward';}
function rewardKindLabel(rewardKey, row) {const category = row?.[5] || '';if (rewardKey === 'suit_stamp' || rewardKey === 'five_stamp') return 'Stamp';if (category === 'relic' || rewardKey === 'relicSlot') return 'Relic';if (category === 'pattern' || category === 'scoring') return 'Scoring';if (category === 'hand') return 'Hand Upgrade';if (category === 'draw') return 'Draw Upgrade';if (category === 'sight') return 'Sight Upgrade';if (category === 'thread') return 'Thread Upgrade';return 'Upgrade';}
...
function renderRewardChoiceCard(bundle, rewardKey, target = window) {
  ...
  return `<div class="store-card store-card--reward-choice reward-role-${escapeHtml(role)}"><div class="store-card-tag">${escapeHtml(rewardKindLabel(rewardKey, row))}</div><div class="store-card-art"><span class="isp isp-108 ${escapeHtml(icon)}"></span></div><div class="store-card-main"><div class="store-card-name">${escapeHtml(row[0])}</div><div class="store-card-desc">${escapeHtml(desc)}</div><div class="store-card-choice-lv">Lv ${level} → ${level + 1}</div></div><button class="store-card-buy" onclick="pickRewardBundleChoice('${escapeHtml(bundle.id)}','${escapeHtml(rewardKey)}')">Take</button></div>`;
}
...
function rewardBundleChoicesInner(bundle, target = window) {
  ...
  return `<div class="store-meta"><button class="store-refresh" onclick="openShopMain()">← Market</button><button class="store-refresh" ${canRefresh ? '' : 'disabled'} onclick="refreshRewardBundleChoices('${escapeHtml(bundle.id)}')"><span class="store-refresh-icon">↻</span> ${canRefresh ? 'Refresh' : 'Refresh Used'}</button></div><div class="store-bundle-note">Choose 1 Reward</div><div class="store-offer-row">${keys.map(key => renderRewardChoiceCard(bundle, key, target)).join('')}</div>`;
}

function renderRewardBundleChoices(bundle, target = window) {ensureBundleStyles(target);const front = target.document?.getElementById('storeFront') || target.document?.querySelector('.store-front-shell .store-front');if (!front) return false;front.innerHTML = rewardBundleChoicesInner(bundle, target);return true;}
...
function transitionToRewardBundleChoices(bundle, target = window) {
  ...
  target.setTimeout?.(() => {
    ...
    target.setTimeout?.(() => {front.classList.remove('bundle-choice-transition', 'bundle-choice-transition-in');target.__tlrRewardChoiceTransitionActive = false;}, 330);
  }, 170);
  return true;
}
...
function trackReason(delta) {return (delta.reasons || []).filter(Boolean).join(' · ') || delta.label;}
function deltaBundleCount(delta) {return Array.isArray(delta.bundleIds) && delta.bundleIds.length ? delta.bundleIds.length : (delta.bundleId ? 1 : 0);}
...
function patchResultsOverlay(target = window) {
  ...
  if (!table || table.querySelector('.bundle-result-row')) return;
...
function trackGhostRunKey(run = {}) {return `${run.reading || 1}:${run.thresholdIndex || 0}:${run.setIndex || 0}`;}
function emptyTrackMap() {return { restless: 0, stillness: 0, sequence: 0, echo: 0, court: 0, thread: 0 };}
function emittedTrackState(target, run) {const key = trackGhostRunKey(run || {});if (!target.__tlrEmittedTrackGhosts || target.__tlrEmittedTrackGhosts.key !== key) target.__tlrEmittedTrackGhosts = { key, emitted: emptyTrackMap() };return target.__tlrEmittedTrackGhosts.emitted;}
function cardRankKey(card) {if (!card) return null;if (card.rank != null) return `rank:${card.rank}`;if (card.number != null) return `num:${card.number}`;return null;}
function echoTrackScoreForCards(cards = []) {const counts = new Map();cards.filter(Boolean).forEach(card => {const key = cardRankKey(card);if (key) counts.set(key, (counts.get(key) || 0) + 1);});const best = Math.max(0, ...counts.values());if (best >= 4) return 3;if (best >= 3) return 2;if (best >= 2) return 1;return 0;}
function sequenceTrackScoreForCards(cards = []) {const values = [...new Set(cards.filter(Boolean).map(card => Number(card.number)).filter(Number.isFinite))].sort((a, b) => a - b);let best = values.length ? 1 : 0;let run = values.length ? 1 : 0;for (let i = 1; i < values.length; i += 1) {run = values[i] === values[i - 1] + 1 ? run + 1 : 1;best = Math.max(best, run);}if (best >= 5) return 3;if (best >= 4) return 2;if (best >= 3) return 1;return 0;}
function courtTrackScoreForCards(cards = []) {return Math.min(5, cards.filter(card => card?.type === 'court').length);}
function openingHandCardsInSpread(run = {}) {const opening = new Set(run.openingHandCardIds || []);return (run.spread || []).filter(card => card && opening.has(card.uid)).length;}
...
function actionTypeName(action) {return String(action?.type || '');}
function actionIs(action, target, actionName) {const type = actionTypeName(action);return type === target.tlrActions?.[actionName] || type.endsWith(actionName);}
function spreadSlotAnchor(slotIndex, target = window) {const index = Number(slotIndex);if (!Number.isInteger(index) || index < 0) return null;return target._slotEls?.[index] || target.document?.querySelectorAll?.('#spread .slot')?.[index] || null;}
function handCardAnchor(cardUid, target = window) {const uid = Number(cardUid);if (!Number.isFinite(uid)) return null;return target.document?.querySelector?.(`#hand .card[data-uid="${uid}"]`) || null;}

function liveGhostOptionsForAction(action, target = window) {
  if (actionIs(action, target, 'PLACE_CARD')) {
    const anchor = spreadSlotAnchor(action.slotIndex, target);
    if (anchor) return { anchor, position: 'above' };
  }
  if (actionIs(action, target, 'DISCARD_SELECTED')) {
    const anchor = handCardAnchor(action.cardUid, target) || target.document?.getElementById?.('discardBtn');
    if (anchor) return { anchor, position: 'above' };
  }
  if (actionIs(action, target, 'MULLIGAN')) {
    const anchor = target.document?.getElementById?.('mullBtn');
    if (anchor) return { anchor, position: 'above' };
  }
  return { position: 'below' };
}

function fireLiteralTrackGhost(target, label, delay = 0, options = {}) {target.setTimeout?.(() => {if (typeof target.fireTrackGhost === 'function') target.fireTrackGhost(label, options);else if (typeof target.centerGhost === 'function') target.centerGhost(label);}, delay);}
function trackGhostLabel(trackId, amount, fallbackLabel = null) {const label = fallbackLabel || TRACK_LABELS[trackId] || trackId;return `+${amount} ${label}`;}
...
function playTrackGhostLabels(labels, done = () => {}, target = window, options = {}) {
  ...
  const ghostOptions = options.blocking ? { center: true } : { ...(options.ghostOptions || {}), center: false };
  labels.forEach((label, index) => fireLiteralTrackGhost(target, label, start + index * step, ghostOptions));
  target.setTimeout?.(() => {if (options.blocking) target.__tlrTrackGhostPlaybackActive = false;done();}, start + labels.length * step + (options.blocking ? 120 : 0));
  return true;
}

export function fireInReadingTrackGhosts(action, _beforeRun, afterRun, target = window) {
  ...
  return playTrackGhostLabels(labels, () => {}, target, { blocking: false, ghostOptions: liveGhostOptionsForAction(action, target) });
}

function resultGhostKey(run = {}) {const ids = run.lastResults?.generatedBundleIds || [];return `${run.lastReadingLedger?.id || run.reading || 'reading'}:${ids.join(',')}:${(run.lastResults?.trackDeltas || []).map(delta => `${delta.trackId}:${delta.gained}:${delta.after}`).join('|')}`;}
...
function openBundleMarketAfterTrackGhosts(target = window) {return playTrackResultsBeforeOverlay(() => openBundleMarket(target), target);}
...
function installResultsPatchObserver(target = window) {
  ...
  const schedule = () => {if (maybeDelayResultsOverlayForTrackGhosts(target)) return;patchResultsOverlay(target);};
